...
const WhatsAppContacts: React.FC = () => {
  ...
  const handleCreateContact = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await whatsappApi.createContact(formData);
      setShowAddModal(false);
      setFormData({ first_name: "", last_name: "", phone_number: "", email: "", group_name: "" });
      fetchContacts();
    } catch (err) {
      ...
      <div className="container-fluid">
          ...
          <div className="col-12">
            <div className="page-title-box d-sm-flex align-items-center justify-content-between">
              <div>
                ...
                <p className="text-muted mb-0 mt-1">Manage audience, segments, and recipient directories</p>
              </div>
        ...
        <div className="card">
          <div className="card-header border-bottom d-flex align-items-center justify-content-between">
            ...
            <span className="badge bg-light text-muted fs-12">Total: {contacts.length} Contacts</span>
          </div>
            ...
            <div className="table-responsive">
              <table className="table align-middle table-nowrap mb-0">
                ...
                <tbody>
                  {loading ? (
                    <tr>
                      <td colSpan={6} className="text-center py-4 text-muted">Loading contacts...</td>
                    </tr>
                  ) : contacts.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="text-center py-4 text-muted">No contacts found.</td>
                    </tr>
                  ) : (
                    contacts.map((c) => (
                      <tr key={c.id}>
                        <td>
                          <div className="d-flex align-items-center gap-2">
                            <div className="avatar-xs rounded-circle bg-success-subtle text-success d-flex align-items-center justify-content-center fw-bold">
                              {(c.first_name || c.phone_number).charAt(0).toUpperCase()}
                            </div>
                            <div>
                              <h6 className="mb-0 fs-14">{c.first_name || ""} {c.last_name || ""}</h6>
                            </div>
                          </div>
                        </td>
                        <td>
                          <span className="badge bg-success-subtle text-success">
                            <i className="ri-whatsapp-line me-1"></i> +{c.phone_number}
                          </span>
                        </td>
                        <td>{c.email || "—"}</td>
                        <td>
                          {c.group_name ? (
                            <span className="badge bg-info-subtle text-info">{c.group_name}</span>
                          ) : (
                            "—"
                          )}
                        </td>
            ...
            <div className="modal-dialog modal-dialog-centered">
              <div className="modal-content">
                <div className="modal-header">
                  <h5 className="modal-title">Add New WhatsApp Contact</h5>
                ...
                <form onSubmit={handleCreateContact}>
                  <div className="modal-body">
                    ...
                    <div className="mb-3">
                      <label className="form-label">Group / Segment</label>
                      <input
                        type="text"
                        className="form-control"
